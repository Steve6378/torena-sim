...
Over a live field the runner step resolves its target lane with the documented rules (`honse-sim/src/primitives/runner/lane.rs`): vision cone, overtake targets, candidate lanes around each target's crowd scored by distance with the early-race outside penalty, the six normal-mode rules, the extra move lane from the final corner, a 1.5 s overtake linger, the overlap bump, and side blocking as the test for whether a move is possible. Front blocking caps speed at 0.988 to 1.0 times the blocker's speed. The doc measures lanes in course widths; the engine keeps meters and scales the doc's constants (lane change speed, the inward drift, the final-corner lane) by the course width where it reads them. Two numbers are named as assumptions in that module: the longitudinal reach of the "inside uma" in normal-mode rule 5 (3 m, the crowd distance) and the pace-down target lane 0.18 read as meters against the doc's unit, because the captures score worse with it in widths (`docs/mechanics/capture-findings.md`). The vacuum engine keeps its approximate lane path; `FieldInputs::lane_neighbors` is `None` there.
...
Scores per run: finish time MAE and bias, winner hit rate, Spearman rank correlation of the finish order, spurt start MAE, skill activation error, and, over the recorded frames, trajectory MAE, speed and HP MAE and bias, and lane MAE. Lane is in meters from the rail; the replay's `lane_position` is 10000 units per course width, as in the game's capture. Frames are matched by time, not index: the engine's replay starts at the first tick while the game records a frame at 0. A per-runner breakdown prints under each pinned run with speed and HP bias per race phase, so an aggregate points at a runner and a phase.
